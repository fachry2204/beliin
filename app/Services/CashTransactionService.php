    ...
    public function syncInvoiceShipping(Invoice $invoice, int $userId, ?bool $paidNow = null): ?CashTransaction
    {
        return DB::transaction(function () use ($invoice, $userId, $paidNow) {
            ...
            $data = [
                ...
                'description' => "Ongkir driver - {$invoice->billing_name}",
                'payment_method' => 'cash',
                'amount' => $invoice->shipping_cost,
    ...
    public function syncFactureShipping(CombinedInvoiceDocument $document, int $userId): ?CashTransaction
    {
        return DB::transaction(function () use ($document, $userId) {
            ...
            $data = [
                ...
                'category' => 'Ongkir Driver',
                'description' => "Ongkir driver - ".($document->customer->company_name ?: $document->customer->name),
                'payment_method' => 'cash',
    ...
    public function syncInvoiceCost(Invoice $invoice, int $userId): ?CashTransaction
    {
        return DB::transaction(function () use ($invoice, $userId) {
            ...
            $data = [
                ...
                'description' => "Harga modal {$invoice->invoice_number} - {$invoice->billing_name}",
                'payment_method' => 'cash',
                'amount' => $invoice->total_cost,
                'reference_number' => $invoice->invoice_number,
                'notes' => "Kas Keluar otomatis dari total harga modal invoice {$invoice->invoice_number}",
                'updated_by' => $userId,
            ];
