...
interface Row { id:number; invoice_number:string; invoice_date:string; billing_name:string; billing_company?:string; subtotal:string; discount_amount:string; grand_total:string; total_cost?:string; gross_profit?:string; paid_amount:string; remaining_amount:string; status:string|{value:string}; }
...
const props = defineProps<{ summary:Summary; rows:PageData; filters:Record<string,string>; canViewProfit:boolean }>();
...
const marginRate=(row:Row)=>{const revenue=Number(row.subtotal)-Number(row.discount_amount||0);return `${Math.round(revenue>0?(Number(row.gross_profit||0)/revenue)*100:0)}%`;};
...
<template>
    <Head title="Laporan Invoice"/><AuthenticatedLayout><template #breadcrumb>Laporan / Invoice</template>
        ...
        <section class="panel"><div class="table-wrap"><table class="data-table min-w-[1150px]"><thead><tr><th>Tanggal</th><th>Nomor Invoice</th><th>Pelanggan</th><th class="text-right">Grand Total</th><th v-if="canViewProfit" class="text-right">Modal</th><th v-if="canViewProfit" class="text-right">Margin</th><th class="text-right">Terbayar</th><th class="text-right">Sisa</th><th>Status</th></tr></thead><tbody><tr v-for="row in rows.data" :key="row.id"><td>{{date(row.invoice_date)}}</td><td><Link :href="route('invoices.show',row.id)" class="font-semibold text-sky-600 hover:underline">{{row.invoice_number}}</Link></td><td>{{row.billing_company||row.billing_name}}</td><td class="text-right">{{money(row.grand_total)}}</td><td v-if="canViewProfit" class="text-right text-amber-600">{{money(row.total_cost||0)}}</td><td v-if="canViewProfit" class="text-right"><div class="font-semibold text-emerald-600">{{money(row.gross_profit||0)}}</div><div class="text-xs text-slate-500">{{marginRate(row)}}</div></td><td class="text-right text-emerald-600">{{money(row.paid_amount)}}</td><td class="text-right font-semibold text-red-600">{{money(row.remaining_amount)}}</td><td><StatusBadge :status="status(row.status)"/></td></tr><tr v-if="!rows.data.length"><td :colspan="canViewProfit?9:7" class="py-12 text-center text-slate-500">Tidak ada invoice sesuai filter.</td></tr></tbody></table></div><div class="flex items-center justify-between border-t p-4"><span class="text-xs text-slate-500">{{rows.from??0}}-{{rows.to??0}} dari {{rows.total}}</span><Pagination :links="rows.links"/></div></section>
    </AuthenticatedLayout>
</template>
